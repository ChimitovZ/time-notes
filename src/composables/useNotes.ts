...
async function fetchNotes(limit: number, offset: number): Promise<PaginatedNotes> {
  const response = await http.get('/notes', { params: { limit, offset } })
...
export function useNotes(limit: MaybeRefOrGetter<number>, page: MaybeRefOrGetter<number>) {
  ...
  const query = useQuery({
    queryKey: computed(() => ['notes', safeLimit.value, offset.value]),
    queryFn: ({ queryKey }) => {
      const [, currentLimit, currentOffset] = queryKey as [string, number, number]
      return fetchNotes(currentLimit, currentOffset)
    },
  })
